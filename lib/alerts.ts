...
async function createAlert(channelId: string, type: string, message: string) {
...
function tier1Share(countries: { country: string; views: number }[], set: string[]): number {
...
export async function runAlertsForChannel(channelId: string) {
  ...
  if (recent.length >= 3) {
    ...
    if (s2 < s1 && s3 < s2) {
      ...
      await createAlert(channelId, 'tier1_drop', msg)
      details.push(msg)
      triggered++
      if (emailBase) {
  ...
  if (lastEntry && prevEntry && lastEntry.videoCount > 0) {
    ...
    if (noNewVideo && daysSincePrev >= 7) {
      ...
      await createAlert(channelId, 'no_video_7d', msg)
      details.push(msg)
      triggered++
  ...
  if (lastEntry) {
    ...
    const ypp1Reached = lastEntry.subscribers >= 500 && lastEntry.watchHours >= 3000
    const ypp2Reached = lastEntry.subscribers >= 1000 && lastEntry.watchHours >= 4000

    if (ypp1Reached) {
      const msg = `🎉 Félicitations ! Palier 1 YPP atteint (${lastEntry.subscribers} abonnés, ${lastEntry.watchHours}h).`
      await createAlert(channelId, 'ypp_reached', msg)
      details.push(msg)
      triggered++
      ...
      await createAlert(channelId, 'ypp_close', msg)
      details.push(msg)
      triggered++
    }
    ...
    if (channelAgeMonths >= 1 && lastEntry.watchHours < expectedHoursAt) {
      ...
      await createAlert(channelId, 'trajectory_low', msg)
      details.push(msg)
      triggered++
    }
  }
